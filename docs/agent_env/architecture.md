...
- training entrypoints: `train.py` and `train_async.py`
...
1. Exports the router URL as `AGENT_ENV_ROUTER_URL` and env-specific URL vars.
2. Sources optional aux endpoint env file.
3. Calls `examples/agent_env/scripts/run_agent_env_train.sh`.

The train adapter translates resolved profiles to a Slime CLI command.
...
The runtime env config is generated by copying the selected env config and
injecting the router URL. This file is an artifact for reproducibility; it is
not a new source-of-truth config layer.
