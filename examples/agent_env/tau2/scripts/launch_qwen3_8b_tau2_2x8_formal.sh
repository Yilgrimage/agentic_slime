#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_DIR="$(cd "${SCRIPT_DIR}/../../../.." && pwd)"

NODES_FILE=${NODES_FILE:-configs/nodes/agent_env_2x8.txt}
ENV_POOL_SIZE=${ENV_POOL_SIZE:-8}
EXP_PROJECT=${EXP_PROJECT:-${PROJECT_NAME:-Qwen3-8B_tau2_grpo}}
EXP_NAME=${EXP_NAME:-${RUN_NAME:-tau2-qwen3-8b-grpo-2x8-formal-gbs64}}
WANDB_PROJECT=${WANDB_PROJECT:-${EXP_PROJECT}}
WANDB_ENTITY=${WANDB_ENTITY:-yilgrimage-bytedance}
WANDB_GROUP=${WANDB_GROUP:-${EXP_NAME}}
TOTAL_NUM_STEPS=${TOTAL_NUM_STEPS:-100}
RESUME_FROM=${RESUME_FROM:-}
RAY_CUDA_VISIBLE_DEVICES=${RAY_CUDA_VISIBLE_DEVICES:-0,1,2,3}
export RAY_CUDA_VISIBLE_DEVICES
EXTRA_LAUNCH_ARGS=()

usage() {
  cat <<'EOF'
Usage: launch_qwen3_8b_tau2_2x8_formal.sh [--dry-run]

Launch a 2-node Qwen3-8B tau2 GRPO run through scripts/mlf/launch_agentic_training.sh.
EOF
}

while [ $# -gt 0 ]; do
  case "$1" in
    --dry-run) EXTRA_LAUNCH_ARGS+=(--dry-run); shift ;;
    -h|--help) usage; exit 0 ;;
    *) echo "Unknown argument: $1" >&2; usage >&2; exit 1 ;;
  esac
done

TRAIN_CMD=$(
  cat <<EOF
SLIME_ENV=/tmp/mlf-envs/slime \
CUDA_VISIBLE_DEVICES=${RAY_CUDA_VISIBLE_DEVICES} \
RAY_CUDA_VISIBLE_DEVICES=${RAY_CUDA_VISIBLE_DEVICES} \
NUM_GPUS=4 \
EXP_PROJECT=${EXP_PROJECT} \
EXP_NAME=${EXP_NAME} \
LOAD_DIR=${RESUME_FROM} \
USE_EXISTING_AGENT_INFRA=1 \
SUBMIT_VIA_RAY_JOB=1 \
ENABLE_WANDB=1 \
WANDB_PROJECT=${WANDB_PROJECT} \
WANDB_ENTITY=${WANDB_ENTITY} \
WANDB_GROUP=${WANDB_GROUP} \
NUM_STEPS=${TOTAL_NUM_STEPS} \
ROLLOUT_BATCH_SIZE=8 \
N_SAMPLES_PER_PROMPT=8 \
GLOBAL_BATCH_SIZE=64 \
ROLLOUT_MAX_CONTEXT_LEN=10240 \
ROLLOUT_MAX_RESPONSE_LEN=512 \
MAX_TOKENS_PER_GPU=20480 \
SGLANG_SERVER_CONCURRENCY=8 \
ACTOR_NUM_NODES=2 \
ACTOR_GPUS=4 \
ROLLOUT_GPUS=8 \
TP_SIZE=4 \
CP_SIZE=1 \
SAVE_INTERVAL=9999 \
bash examples/agent_env/tau2/scripts/run_qwen3_8b_tau2_grpo.sh
EOF
)

cd "${REPO_DIR}"
bash scripts/mlf/launch_agentic_training.sh \
  --env tau2 \
  --nodes "${NODES_FILE}" \
  --env-pool-size "${ENV_POOL_SIZE}" \
  --env-port 18182 \
  --router-port 19082 \
  --train-cmd "${TRAIN_CMD}" \
  ${EXTRA_LAUNCH_ARGS+"${EXTRA_LAUNCH_ARGS[@]}"}
