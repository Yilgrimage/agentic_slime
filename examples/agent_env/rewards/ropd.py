...
- `count` 只表示同名 API 在该 step 中被调用的次数，不证明这些调用覆盖了不同目标。`distinct_argument_count` 和 `argument_samples` 可证明目标调用的多样性，但“每个/全部”仍需集合规模、目标列表或其它可见结果支持完整覆盖；证据不足时必须保持 milestone 为 false。
