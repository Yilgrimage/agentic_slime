...
def load_tasks(config_path: Path) -> list[dict]:
    ...
    tasks = openclaw_cfg.get("tasks") or cfg.get("tasks") or []
    if not isinstance(tasks, list):
        raise ValueError("openclaw.tasks must be a list")
    task_file = str(openclaw_cfg.get("task_file") or cfg.get("task_file") or "").strip()
    if task_file:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create OpenClaw prompt data for slime rollouts/eval.")
    ...
    parser.add_argument("--config", default="examples/agent_env/openclaw/env_config.yaml")
