...
async def _generate_rollout_fully_async_glm_padding(args: Any, rollout_id: int, data_buffer: Any):
    ...
    padding_offset = 0
    max_seen_groups = int(
        _runtime_env(args, "AGENT_ENV_GLM_PADDING_MAX_SEEN_GROUPS", str(max(target * 16, target + 32)))
    )
    ...
    while len(collected) < target:
        ...
        seen_groups = int(metrics.get("agent_env/glm_padding/group_seen", 0))
        if seen_groups >= max_seen_groups and len(collected) < target:
            raise RuntimeError(
                "GLM-style rollout padding could not collect enough valid groups: "
                f"collected={len(collected)} target={target} seen={seen_groups} "
                f"dropped={int(metrics.get('agent_env/glm_padding/group_dropped', 0))} "
                f"padded={int(metrics.get('agent_env/glm_padding/group_padded', 0))} "
                f"invalid_samples={int(metrics.get('agent_env/glm_padding/invalid_samples', 0))}. "
                "Increase env quality/capacity or AGENT_ENV_GLM_PADDING_MAX_SEEN_GROUPS."
            )
        ...
        if now - last_log > 30.0:
            logger.info(
                "agent-env GLM-style rollout %d: collected %d/%d, queue=%d, seen=%d, dropped=%d, padded=%d, elapsed=%.1fs",
                rollout_id,
                len(collected),
                target,
                worker.queue_size(),
                int(metrics.get("agent_env/glm_padding/group_seen", 0)),
                int(metrics.get("agent_env/glm_padding/group_dropped", 0)),
                int(metrics.get("agent_env/glm_padding/group_padded", 0)),
                now - started,
            )
            last_log = now
    ...
    logger.info(
        ...
        int(metrics.get("agent_env/glm_padding/group_dropped", 0)),
        int(metrics.get("agent_env/glm_padding/group_padded", 0)),
        int(metrics.get("agent_env/glm_padding/padded_samples", 0)),
    )
    return RolloutFnTrainOutput(samples=output, metrics=metrics)


def generate_rollout_fully_async_glm_padding(args: Any, rollout_id: int, data_buffer: Any, evaluation: bool = False):
