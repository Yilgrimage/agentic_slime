...
EXP_PROJECT=${EXP_PROJECT:-${PROJECT_NAME:-Qwen3-8B_appworld_grpo}}
EXP_NAME=${EXP_NAME:-${RUN_NAME:-appworld-qwen3-8b-grpo-2x8-formal-gbs64}}
...
export RAY_CUDA_VISIBLE_DEVICES
EXTRA_LAUNCH_ARGS=()

usage() {
  cat <<'EOF'
Usage: launch_qwen3_8b_appworld_2x8_formal.sh [--dry-run]

Launch a 2-node Qwen3-8B AppWorld GRPO run through scripts/mlf/launch_agentic_training.sh.

Environment overrides:
  NODES_FILE              Node list, default configs/nodes/agent_env_2x8.txt
  ENV_POOL_SIZE           AppWorld env processes per node, default 8
  EXP_PROJECT             Local/W&B project, default Qwen3-8B_appworld_grpo
