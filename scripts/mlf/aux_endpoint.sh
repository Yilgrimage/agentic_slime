...
SSH_KEY=${SSH_KEY:-/home/${SSH_USER}/.ssh/byte_id_rsa}
SSH_IPV6=${SSH_IPV6:-1}
...
apply_aux_defaults() {
  ...
  AUX_PP=${AUX_PP:-1}
  AUX_SESSION=${AUX_SESSION:-mlf_tau2_aux}
  ...
  AUX_SERVED_MODEL_NAME=${AUX_SERVED_MODEL_NAME:-tau2-user-sim}
...
sys.exit(0 if ok else 1)"
  ...
  if is_current_node "${host}"; then
    "${AUX_PYTHON}" -c "${py_code}"
  else
    remote_query "${host}" "$(printf '%q ' "${AUX_PYTHON}" -c "${py_code}")"
  fi
}
...
resolve_aux_node() {
  ...
  if [ -z "${AUX_NODE}" ]; then
    ...
    AUX_NODE=$(awk 'NF && $1 !~ /^#/ {print $1; exit}' "${nodes_file}")
  fi
  printf '%s\n' "${AUX_NODE}"
}
...
cleanup_aux_runtime_cmd() {
  cat <<EOF
tmux kill-session -t mlf_tau2_aux 2>/dev/null || true
tmux kill-session -t mlf_aux_endpoint 2>/dev/null || true
...
start_sglang_endpoint() {
  ...
  if [ "${DRY_RUN}" = "0" ] && [ ! -x "${AUX_PYTHON}" ]; then
    echo "Missing aux inference python: ${AUX_PYTHON}" >&2
    echo "Prepare the slime runtime first, for example: bash scripts/mlf/prepare_agentic_runtime.sh --envs slime --data none --sources none" >&2
    exit 1
  fi
  nodes_file=$(resolve_path "${AUX_NODES_FILE}")
  model_path=$(resolve_local_model_path)
  node=$(resolve_aux_node "${nodes_file}")
  if [ "${DRY_RUN}" = "0" ] && [ "${BENCH_ON_AUX_START}" = "1" ] && [ -f "${MLF_NAS_ROOT}/bash/run_bench.sh" ]; then
  ...
  serve_cmd=$(printf 'cd %q && mkdir -p %q && export PYTHONNOUSERSITE=1 CUDA_VISIBLE_DEVICES=%q PYTHONPATH=%q no_proxy=%q NO_PROXY=%q && %q -m sglang.launch_server --model-path %q --served-model-name %q --host 0.0.0.0 --port %q --tp-size %q --mem-fraction-static %q --reasoning-parser %q --tool-call-parser %q --trust-remote-code > %q 2>&1' \
    ...
    "${AUX_PYTHON}" "${model_path}" "${AUX_SERVED_MODEL_NAME}" "${AUX_PORT}" "${tp}" "${AUX_MEM_FRACTION}" \
  ...
  if is_current_node "${node}"; then
    if [ "${DRY_RUN}" = "1" ]; then
      ...
      write_env_file "sglang" "${AUX_SERVED_MODEL_NAME}" "${base_url}" "" 1 "${nodes_file}"
...
start_vllm_endpoint() {
  ...
  if [ "${DRY_RUN}" = "0" ] && [ ! -x "${AUX_PYTHON}" ]; then
    echo "Missing aux inference python: ${AUX_PYTHON}" >&2
    echo "Prepare the aux inference runtime first, for example: /mnt/bn/jixf-nas-lq/mlf/envs/aux-infer" >&2
    exit 1
  fi
  nodes_file=$(resolve_path "${AUX_NODES_FILE}")
  model_path=$(resolve_local_model_path)
  ...
  tp="${AUX_TP:-$(infer_tp_from_model "${model_path}")}"
  log="${LOG_DIR}/aux_endpoint.log"
  vllm_bin="${AUX_VLLM_BIN:-$(dirname "${AUX_PYTHON}")/vllm}"
  ...
  if is_current_node "${node}"; then
    if [ "${DRY_RUN}" = "1" ]; then
      printf '+ tmux new-session -d -s %q %q\n' "${AUX_SESSION}" "${serve_cmd}"
      wait_openai_models "${node}" "$(health_url "${base_url}")"
      write_env_file "vllm" "${AUX_SERVED_MODEL_NAME}" "${base_url}" "" 1 "${nodes_file}"
      echo "aux endpoint log: ${node}:${log}"
      return 0
    fi
    tmux kill-session -t "${AUX_SESSION}" 2>/dev/null || true
