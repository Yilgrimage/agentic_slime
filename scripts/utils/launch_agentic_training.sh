...
RESOLVED_TRAIN_EXTRA_KEYS=(
  ...
  ACTOR_SAVE_DIR CRITIC_SAVE_DIR
  AGENT_ENV_DATA_DIR
  NO_LOAD_OPTIM NO_LOAD_RNG FINETUNE
  USE_CHECKPOINT_OPT_PARAM_SCHEDULER OVERRIDE_OPT_PARAM_SCHEDULER
  ...
  PROMPT_NUM_TASKS PROMPT_DATA_EXTRA_ARGS
  EVAL_CONFIG EVAL_INTERVAL EVAL_FUNCTION_PATH EVAL_PROMPT_DATA EVAL_SPLITS EVAL_DATA_DIR
  EVAL_PROMPT_NUM_TASKS EVAL_PROMPT_DATA_SCRIPT EVAL_PROMPT_DATA_PYTHON EVAL_PROMPT_DATA_CONFIG
  EVAL_PROMPT_DATA_EXTRA_ARGS EVAL_MAX_RESPONSE_LEN EVAL_TEMPERATURE EVAL_TOP_P EVAL_TOP_K
  SKIP_EVAL_BEFORE_TRAIN
  LR_DECAY_ITERS
  DYNAMIC_SAMPLING_FILTER_PATH ROLLOUT_SAMPLE_FILTER_PATH
...
ray_start_script() {
  ...
  cmd+=(--disable-usage-stats --block)
  {
    ...
    printf '%q -c %q\n' "${SLIME_PYTHON}" "import sys; print('ray_python', sys.executable)"
    if [ "${ENABLE_WANDB:-0}" = "1" ] || [ "${USE_WANDB:-0}" = "1" ]; then
      quote_export_vars WANDB_RUNTIME WANDB_PACK_NAME WANDB_PACK_PATH SLIME_PYTHON
      printf 'source %q\n' "${SCRIPT_DIR}/slime_runtime.sh"
      printf 'resolve_wandb_runtime\n'
      printf 'export PYTHONPATH="${WANDB_PYTHONPATH}${PYTHONPATH:+:${PYTHONPATH}}"\n'
    fi
...
main_eval_sweep() {
  ...
  for i in "${!steps[@]}"; do
    ...
    local child_cmd=(
      ...
      # Native eval uses eight TP1 rollout engines by default. Leave capacity
      # below the 48-worker AppWorld pool for long multi-turn episodes.
      "SGLANG_SERVER_CONCURRENCY=${EVAL_SGLANG_SERVER_CONCURRENCY:-4}"
